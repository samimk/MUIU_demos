...
class TicTacToeML:
    ...
    def __init__(self):
        ...
        self.weights = {
            'my_pieces': 0.5,           # w1: broj mojih figura
            'opp_pieces': 0.5,         # w2: broj protivničkih figura
            'my_two_in_row': 0.5,       # w3: moje 2 u redu
            'opp_two_in_row': 0.5,     # w4: protivničke 2 u redu
            'center': 0.5,              # w5: kontrola centra
            'corners': 0.5              # w6: kontrola uglova
    ...
    def train(self, num_games: int = 100):
        """
        4. ALGORITAM UČENJA - Self-play trening
        
        AI igra protiv samog sebe i ažurira težine na osnovu rezultata.
        Ovo je pojednostavljena verzija - pravi RL bi koristio TD-learning.
        """
        ...
        for game in range(num_games):
            winner = self.play_game(player1='ai', player2='ai', verbose=False)
            
            # Jednostavno ažuriranje: pojačaj težine ako O pobijedi
            # (u realnosti bi koristili sofisticiraniji algoritam)
            if winner == 'O':
                self.weights['my_two_in_row'] *= 1.01
                self.weights['opp_two_in_row'] *= 1.01
        ...
        for key in self.weights:
            ...
            print(f"  {key}: {initial_weights[key]:.3f} → {self.weights[key]:.3f} "
                  f"(Δ = {change:+.3f})")
